...
const ServicesTab = () => {
  ...
  const handleAddService = (e) => {
    e.preventDefault();
    if (!newService.name || !newService.duration || !newService.price || !newService.description) {
  ...
  const handleEditServiceSubmit = (e) => {
    e.preventDefault();
    if (!editServiceForm.name || !editServiceForm.duration || !editServiceForm.price) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Add Service Drawer */}
      <DetailDrawer open={addServiceOpen} onOpenChange={setAddServiceOpen} title="Add Service">
        <form onSubmit={handleAddService} className="space-y-4 p-4">
          ...
          <div>
            ...
            <div className="space-y-2">
              {newService.benefits.map((benefit, index) => (
                <div key={index} className="flex gap-2">
                  ...
                  <Button
                    ...
                    size="sm"
                    onClick={() => {
                      const newBenefits = newService.benefits.filter((_, i) => i !== index);
                      setNewService({ ...newService, benefits: newBenefits });
                    }}
                  >
                    Remove
              ...
              <Button
                type="button"
                variant="outline"
                onClick={() => setNewService({ ...newService, benefits: [...newService.benefits, ''] })}
              >
                Add Benefit
              </Button>
            </div>
            ...
            <div className="flex flex-wrap gap-2">
              {allProducts.map(product => (
                <button
                  type="button"
                  key={product}
                  className={`px-3 py-1 rounded-full border text-xs ${newService.products.includes(product) ? 'bg-glamour-700 text-white border-glamour-700' : 'bg-white text-glamour-800 border-gray-300'}`}
          ...
          {addServiceError && <div className="text-red-600 text-sm">{addServiceError}</div>}
          <Button type="submit" className="bg-glamour-700 hover:bg-glamour-800 text-white w-full">Add Service</Button>
        </form>
      </DetailDrawer>
      {/* Edit Service Drawer */}
      <DetailDrawer open={editServiceOpen} onOpenChange={setEditServiceOpen} title="Edit Service">
        <form onSubmit={handleEditServiceSubmit} className="space-y-4 p-4">
          ...
          <div>
            ...
            <div className="space-y-2">
                ...
                <div key={index} className="flex gap-2">
                  <Input
                    ...
                    onChange={e => {
                      ...
                      newBenefits[index] = e.target.value;
                      setEditServiceForm({ ...editServiceForm, benefits: newBenefits });
                    }}
                  />
                  <Button
                    type="button"
                    ...
                    size="sm"
                    onClick={() => {
                      const newBenefits = editServiceForm.benefits.filter((_, i) => i !== index);
                      setEditServiceForm({ ...editServiceForm, benefits: newBenefits });
                    }}
                  >
                    Remove
              ...
              <Button
                type="button"
                variant="outline"
                onClick={() => setEditServiceForm({ ...editServiceForm, benefits: [...editServiceForm.benefits, ''] })}
              >
                Add Benefit
              </Button>
            </div>
            ...
            <div className="flex flex-wrap gap-2">
              {allProducts.map(product => (
                <button
                  type="button"
                  key={product}
                  className={`px-3 py-1 rounded-full border text-xs ${editServiceForm.products.includes(product) ? 'bg-glamour-700 text-white border-glamour-700' : 'bg-white text-glamour-800 border-gray-300'}`}
          ...
          {editServiceError && <div className="text-red-600 text-sm">{editServiceError}</div>}
