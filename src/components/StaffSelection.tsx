...
const StaffSelection: React.FC<StaffSelectionProps> = ({ 
  ...
  // Debug logging
  console.log('StaffSelection render:', {
    serviceId,
  ...
  if (!staff || staff.length === 0) {
    return (
      <div className="space-y-2">
        ...
        <p className="text-xs text-gray-400">Debug: staff array is {staff ? 'empty' : 'null/undefined'}</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-2">
      <p className="text-sm font-medium text-gray-700">{t('booking.selectStaff')}</p>
      <p className="text-xs text-gray-400">Found {staff.length} staff member(s)</p>
      <div className="grid grid-cols-1 gap-2">
        {staff.map((staffMember) => {
          ...
          return (
            <Card 
              ...
              onClick={() => {
                console.log('Staff clicked:', staffMember.id, staffMember.full_name);
                onStaffSelect(staffMember.id, staffMember.full_name || staffMember.name || 'Unknown Staff');
              }}
            >
