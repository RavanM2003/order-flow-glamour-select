...
const About = () => {
  ...
  const { data: staffWithServices, isLoading: staffLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      // Convert Json types to our TypeScript interfaces with proper type casting
      return (data || []).map(
        (item): StaffData => ({
          user: item.user as unknown as StaffUser,
          positions: item.positions as unknown as StaffPosition[],
        })
      );
    },
  });
  ...
  const ourStoryTitle = getLocalizedSetting(
    "about_our_story_title",
    "Our Story"
  );
  const ourStoryContent = getLocalizedSetting(
    "about_our_story_content",
    "about_us"
  );
  const meetOurTeamTitle = getLocalizedSetting(
    "about_team_title",
    "Meet Our Team"
  );
  const ourValuesTitle = getLocalizedSetting(
    "about_values_title",
    "Our Values"
  );

  // Values from settings
  const valueQualityTitle = getLocalizedSetting(
    "value_quality_title",
    "Quality"
  );
  const valueQualityText = getLocalizedSetting(
    "value_quality_text",
    "We use only the highest quality products and maintain the strictest standards in all our services."
  );
  const valueCareTitle = getLocalizedSetting("value_care_title", "Care");
  const valueCareText = getLocalizedSetting(
    "value_care_text",
    "Every client receives personalized attention and care tailored to their unique needs and preferences."
  );
  const valueExcellenceTitle = getLocalizedSetting(
    "value_excellence_title",
    "Excellence"
  );
  const valueExcellenceText = getLocalizedSetting(
    "value_excellence_text",
    "We strive for excellence in every service, ensuring you leave feeling confident and beautiful."
  );
